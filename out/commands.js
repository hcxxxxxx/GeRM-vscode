...
const codeAnalyzer_1 = require("./core/codeAnalyzer");
const readmeGenerator_1 = require("./core/readmeGenerator");
    ...
    const generateReadmeCommand = vscode.commands.registerCommand('germ.generateReadme', async () => {
        ...
        try {
            vscode.window.withProgress({
                ...
                // 分析代码仓库
                const codeAnalyzer = new codeAnalyzer_1.CodeAnalyzer(workspacePath);
                const repoAnalysis = await codeAnalyzer.analyze();
                progress.report({ message: "生成README内容...", increment: 33 });
                // 生成README
                const readmeGenerator = new readmeGenerator_1.ReadmeGenerator();
                ...
                try {
                    ...
                    // 如果README存在，询问是否覆盖
                    const answer = await vscode.window.showWarningMessage('README.md已存在，是否覆盖?', { modal: true }, '覆盖', '取消');
                    if (answer !== '覆盖') {
